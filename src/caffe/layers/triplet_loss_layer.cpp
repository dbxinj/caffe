...
void TripletLossLayer<Dtype>::Forward_cpu(
  ...
  Dtype loss(0.0);
  for (int i = 0; i < bottom[0]->num(); ++i) {
    diff_sq_ap_.mutable_cpu_data()[i] = caffe_cpu_dot(channels,
        diff_ap_.cpu_data() + (i*channels), diff_ap_.cpu_data() + (i*channels));
    diff_sq_an_.mutable_cpu_data()[i] = caffe_cpu_dot(channels,
        diff_an_.cpu_data() + (i*channels), diff_an_.cpu_data() + (i*channels));
    Dtype mdist = sample[i]*std::max<Dtype>(margin + diff_sq_ap_.cpu_data()[i] - diff_sq_an_.cpu_data()[i], Dtype(0.0));
    loss += mdist;
    if(mdist == Dtype(0.0)) {
