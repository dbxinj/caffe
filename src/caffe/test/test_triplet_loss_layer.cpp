...
class TripletLayerTest : public MultiDeviceTest<TypeParam> {
...
TYPED_TEST(TripletLossLayerTest, TestForward) {
  ...
  const Dtype* sample = this.blob_bottom_y_->cpu_data();
  Dtype loss(0);
  for (int i = 0; i < num; ++i) {
    Dtype dist_sq_ij(0);
    Dtype dist_sq_ik(0);
    ...
    loss += sample[i] * std::max(margin+dist_sq_ij-dist_sq_jk, Dtype(0));
  }
