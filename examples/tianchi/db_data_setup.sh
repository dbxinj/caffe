...
if [ $# -lt 4 ]; then
	echo "Usage: "
	echo "  ./db_data_setup.sh [convert|extract] src_dir dst_dir batch_num [label_dir]"
        echo "-----------------------------------------------------------------------------"
        echo "Fuctions:"
	echo "  to set up an index od dataset used for training or extracting feature"
	echo "Parameters:"
	echo "  [convert|extract]: choose one mode, convert for converting image to lmdb" 
	echo "                   and extract for extracting features"
	echo "  in convert mode, dst contains each image name"
	echo "  in extract mode, dst contains each image path" 
...
if [ $1 == "convert" ]; then
	#if [ $# -lt 5 ]; then
	#	echo "Error: more paramters needed for convert mode"
